Ext.define('myschoolishness.store.AbsenceAllStudentStore', {
    extend: 'Ext.data.Store',
	config: {
     	model: 'myschoolishness.model.AbsenceAllStudentModel',
     	autoLoad: true,
	...
	onStoreLoad: function(store, records, successful, operation, eOpts) {
			for (var i=0;i<records.length;i++) {
					...
					var typeStr = myschoolishness.controller.Utils.absenceTypeIntToString(records[i].get("type"));
					var excused = "unexcused";
					if (records[i].get("parent_entered")===1) {
						excused="excused";
					}
					
					records[i].set("displayTypeExc",typeStr + " - " + excused);
				}
     }
